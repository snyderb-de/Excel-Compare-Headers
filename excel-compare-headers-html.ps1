...
if ($dialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    if ($dialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        $output_file = Join-Path (Split-Path $compare_files[0]) "output.html"

        # Create output file
        $output = New-Object System.Collections.ArrayList
        $output.Add("<html><head><title>Header Comparison Results</title></head><body>")
        ...
        $base_headers = @()
        $output.Add("<h2>Base File Headers</h2>")
        $output.Add("<table><tr><th>Column Letter</th><th>Header Value</th></tr>")
        for ($i = 1; $i -le $base_ws.UsedRange.Columns.Count; $i++) {
            $header = $base_ws.Cells.Item(1, $i).Value()
            $column_letter = [char](64 + $i)
            $output.Add("<tr><td>$column_letter</td><td>$header</td></tr>")
            $base_headers += $header
        }
        $base_wb.Close()
    }
